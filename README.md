...
- **Local library** — import audio files via the file picker or drag-and-drop; everything is stored under a single library folder you control
...
- **Real playback engine** — play/pause, next, previous (with true history — previous returns to what actually played, even under shuffle), seek, volume (persisted), played-song tracking per queue, auto-skip past broken files. Browsing never interrupts it: switching the sidebar view keeps the music playing, and the top bar's play button pauses and resumes the view it's already playing instead of restarting it. Scrubbing the seek slider is silent until you let go
...
- **Settings:** the storage manager above, compress-by-default for downloads, and an "Update yt-dlp" button that downloads the latest yt-dlp release into the app's data folder (used in preference to the bundled one afterward — handy when a site stops downloading). The dialog closes with **Ok**; every setting in it takes effect the moment you change it.
