...
- **Sorting & reordering** — the sort menu in the top bar orders the current view (Library or playlist) by **Title**, **Date added**, **Duration**, or **Size**, each direction-toggling on a second click, with **Custom Order** to return to the stored order; the bar names the sort in force and the menu lists modes most-recently-used first. **Under Custom Order, drag songs up and down the list to rearrange the stored order itself** — an accent seam shows where the song will land, and the order is saved. The play queue follows the sort — resorting or reordering mid-listen changes what comes next without interrupting the song
...
- **Sorting & reordering songs:** the sort icon (left of the download icon) offers Custom Order, Title, Date Added, Duration, and Size — click a field to sort ascending, click it again to flip; the caption beside the icon names the sort in force. The sort applies to whichever view you're in and carries into the play queue; it never rewrites the stored order, and it resets to Custom Order on relaunch. Under Custom Order, drag a song up or down the list to rewrite the stored order itself — an accent seam shows which edge it will land on, the change is saved, and a playing queue picks up the new order behind the current song. Dragging pauses while a search narrows the list or a row's menu is open.
